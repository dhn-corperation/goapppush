...
func resultProcess(wg *sync.WaitGroup){
	...
	select 
		...
		a.extra2,
		b.profile
	from ` + cf.Conf.APP_RESPONSE_TABLE + ` a 
	left join DHN_REQUEST_APP b on a.extra2 = b.msgid 
	where 
		a.extra3 = '' and a.step in ('C', 'F')`
	...
	if isProc {
		for groupRows.Next() {
			var queueidx, msgidx, step, error_code, resultdate, extra2, profile sql.NullString
			resCode := ""

			groupRows.Scan(&queueidx, &msgidx, &step, &error_code, &resultdate, &extra2, &profile)

			if s.EqualFold(step.String, "C") && len(profile.String) > 0 {
				cf.Stdlog.Println(1)
			} else if (s.EqualFold(step.String, "F") && len(profile.String) > 0){
				resCode = error_code.String
				cf.Stdlog.Println(2)
			} else if s.EqualFold(step.String, "C") && len(profile.String) <= 0 {
				resCode = "0000"
				cf.Stdlog.Println(3)
			} else if (s.EqualFold(step.String, "F") && len(profile.String) <= 0){
				resCode = error_code.String
